...
namespace Globeport.Client.Uwp.Sdk.Crypto
{
    public class UwpCryptoClient : CryptoClient
    {
        ...
        DataProtectionProvider DataProtectionProvider { get; } = new DataProtectionProvider("LOCAL = user");
        ...
        public const int MEMORY_COST = 16384;

        public UwpCryptoClient()
        {
        ...
        {
            var parts = ByteUtil.split(signature, 64, 20);
            signature = parts[0];
        ...
        {
            return Sodium.Core.GetRandomBytes(16);
        }

        public override byte[] GeneratePasswordKey(string password, byte[] salt)
        {
            var options = Sodium.PasswordHash.CreateOptions(MEMORY_COST, TIME_COST);
            return Sodium.KDF.Argon2i(password, salt, options);
        }

        public async override Task<byte[]> Protect(byte[] data)
        {
            var protectedData = await DataProtectionProvider.ProtectAsync(data.AsBuffer()).AsTask().ConfigureAwait(false);

            return protectedData.ToArray();
        }

        public async override Task<byte[]> Unprotect(byte[] data)
        {
            var unprotectedData = await DataProtectionProvider.UnprotectAsync(data.AsBuffer()).AsTask().ConfigureAwait(false);
        ...
        public override void GetRandomBytes(byte[] bytes)
        {
            RandomGenerator.GetRandomBytes(bytes);
        }
