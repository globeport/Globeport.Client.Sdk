namespace Globeport.Shared.Library.Chakra.Hosting
{
    using System;
                    ...
                    case JavaScriptErrorCode.ScriptException:
                        {
                            ...
                            string message = "Script threw an error";
                            string stack = "";

                            if (errorObject.ValueType == JavaScriptValueType.Error)
                            {
                                var id = JavaScriptPropertyId.FromString("message");
                                var property = errorObject.GetProperty(id);
                                if (property.ValueType == JavaScriptValueType.String) message = property.ToString();

                                id = JavaScriptPropertyId.FromString("stack");
                                property = errorObject.GetProperty(id);
                                if (property.ValueType == JavaScriptValueType.String) stack = property.ToString();
                            }

                            throw new JavaScriptScriptException(error, errorObject, message+" "+stack);
                        }
